...
- "naturalExpressions[].chunk": Extracted DIRECTLY from natural. Collocation / phrasal verb / idiom / set phrase / discourse marker. Replace variable content with ~. Keep 3-8 words. NEVER write a full sentence. NEVER output a single isolated word — even with ~ (minimum 3 words required). BAD (forbidden — single word alone): 「potential」「fluent」「natural」BAD (forbidden — single word + ~): 「be ~」「is ~」「are ~」「seem ~」「look ~」「feel ~」「think ~」「get ~」BAD (forbidden — grammar skeleton only): 「It's ~」「from ~ to ~」「I ~ ~」「the ~ of ~」GOOD: 「run into ~ issues」「It might be worth ~ing」「Having said that, ~」「be open to ~ing」「make sure to ~」「have great potential」
