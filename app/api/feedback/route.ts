...
- "naturalExpressions[].chunk": Extracted DIRECTLY from natural. Collocation / phrasal verb / idiom / set phrase / discourse marker. Replace variable content with ~. Keep 3-8 words. NEVER write a full sentence. NEVER use a single verb + ~ only. BAD (forbidden — single word + ~): 「be ~」「is ~」「are ~」「seem ~」「look ~」「feel ~」「think ~」「get ~」BAD (forbidden — grammar skeleton only): 「It's ~」「from ~ to ~」「I ~ ~」「the ~ of ~」GOOD: 「run into ~ issues」「It might be worth ~ing」「Having said that, ~」「be open to ~ing」「make sure to ~」
